...
function renderPresetLoader() {
  ...
  $$('[data-delete]').forEach(button => button.onclick = () => { const saved = getUserPatches(); const deleted = saved.splice(Number(button.dataset.delete), 1)[0]; setUserPatches(saved); populatePresets(); renderPresetLoader(); showToast(`${deleted?.name || 'Custom preset'} deleted`); });
  $$('[data-protected]').forEach(button => button.onclick = () => showToast('Factory presets are protected. Save a custom copy to manage it.'));
}
